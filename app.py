...
def get_insights(recipe: dict, client: OpenAI) -> RecetaInsights:
    ...
    completion = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "Eres un chef experto en cocina mexicana. Analiza la receta y extrae insights utiles en espanol. Responde SOLO con un objeto JSON valido, sin texto adicional."},
            {"role": "user", "content": f"Analiza esta receta y devuelve un JSON con los campos: nombre, tiempo_minutos, dificultad, ingredientes_clave, resumen, consejo_chef, sustituciones, apto_para, puntuacion_facilidad.\n\n{recipe_text}"}
        ],
        response_format={"type": "json_object"}
    )
    data = json.loads(completion.choices[0].message.content)
    return RecetaInsights(**data)
