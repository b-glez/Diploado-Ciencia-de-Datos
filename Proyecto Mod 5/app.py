...
def get_insights(recipe: dict, client: OpenAI) -> RecetaInsights:
    ...
    completion = client.beta.structured_outputs.parse(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "Eres un chef experto en cocina mexicana. Analiza la receta y extrae insights útiles en español."},
            {"role": "user", "content": f"Analiza esta receta:\n\n{recipe_text}"}
        ],
        response_format=RecetaInsights
    )
